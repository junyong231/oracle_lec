...
-- SCOTT에서 기본 SELECT 문을 익히기 위해 테이블을 생성하자
-- 1) sql plus 실행해서 SYS 계정으로 접속하기
-- 2) c:\\oraclexe 폴더에서 scott.sql 검색
SELECT *
FROM TABS;
-- 오라클은 사용자 계정을 생성하면 동일한 이름으로  OB객체를 생성한다 -> 스키마
...
--DQL : SELECT문
WITH    -- (1) 제일 먼저 처리됨
SELECT  -- (6)      DISTINCT, ALL, AS  키워드..
FROM    -- (2)
WHERE   -- (3)
GROUP BY --(4)
HAVING  -- (5)
ORDER BY -- (7) 가장 늦게 처리됨 (오토바이ㅋㅋㅋ는 위험하니까 늦게 혹은 타지말자 ^^)
;
--emp 테이블의 사원 정보 조회

--SELECT empno,ename,job, mgr, hiredate, sal, comm, deptno 
SELECT *
From emp; -- 현재 접속자가 scott이니까 생략 가능
--FROM scott.emp; -- 스키마.테이블명 또는 스키마.뷰명

--scott 계정이 소유하고 있는 테이블 정보 조회
SELECT *
FROM user_tables; -- 내가 만든 테이블 (내차)
FROM dba_tables; -- 오류

FROM all_tables; -- 내가 접근 가능한 테이블 (가족차)
FROM tabs;
...
--emp 테이블의 모든 사원 정보 출력
SELECT *
FROM emp;

--사원번호, 사원명, 잡 만 보고 싶다
SELECT empno, ename, job -- [ALL] 이 생략된거임
FROM emp;

--사원들이 속해있는 job 칼럼만 출력?
SELECT COUNT ( DISTINCT job ) AS 직업수 -- DISTINCT 중복제거! AS 생략가능
FROM emp
;
...
INSERT INTO insa (num, name, ssn, ibsaDate, city, tel, buseo, jikwi, basicPay, sudang) VALUES
  (1060, '김신애', '810809-2111111', '2001-10-10', '서울', '011-4151-4444', '개발부', 
   ...
   --반드시 DML에서는 commit 이나 rollback 해야함
COMMIT;
