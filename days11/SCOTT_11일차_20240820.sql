...
-- 문제점: 0명 고른 선택지는 안나옴...
    ...
    SELECT  e.ans 번호, e.고른수 ,  lpad ( ' ' , e.고른수+1, '*')  그래프,  ROUND( CAST(e.고른수 AS FLOAT) / SUM(CAST(e.고른수 AS FLOAT)) OVER (), 2 ) * 100 || '%' AS 비율
    FROM (
...
FROM answers;
    ...
    SELECT  e.ans 번호, e.고른수 ,  lpad ( ' ' , ROUND( CAST(e.고른수 AS FLOAT) / SUM(CAST(e.고른수 AS FLOAT)) OVER (), 2 ) * 100, '*')  그래프,  ROUND( CAST(e.고른수 AS FLOAT) / SUM(CAST(e.고른수 AS FLOAT)) OVER (), 2 ) * 100 || '%' AS 비율
    FROM (
    SELECT a.ans, COUNT(a.ans) "고른수"
    FROM answers a 
    WHERE a.sul_no =84 --이것만 선택할 수 있으면 ? (ex. 2번 설문 상세보기 클릭시 여기가 2로 되게)
    GROUP BY a.ans
    ORDER BY a.sul_no
    ) e
    ORDER BY 번호;
